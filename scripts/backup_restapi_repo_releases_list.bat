...
set "CURL_OUTPUT_FILE=%GH_BACKUP_OUTPUT_TEMP_DIR%/%GH_RESTAPI_REPO_RELEASES_FILE%"
...
call set "GH_BACKUP_RESTAPI_REPO_RELEASES_FILE=%%GH_BACKUP_RESTAPI_REPO_RELEASES_FILE_NAME:{{OWNER}}=%OWNER%%%"
...
call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/add_files_to_archive.bat" "%%GH_BACKUP_TEMP_DIR%%" "*" "%%GH_BACKUP_OUTPUT_DIR%%/%%GH_BACKUP_RESTAPI_REPO_RELEASES_FILE%%.7z" -sdel || exit /b 20
echo.
