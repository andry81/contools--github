...
set "GH_BACKUP_OUTPUT_TEMP_DIR=%GH_BACKUP_TEMP_DIR%/repos/starred/%OWNER%"
set "GH_BACKUP_OUTPUT_DIR=%GH_BACKUP_RESTAPI_STARRED_REPOS_DIR%/%OWNER%"
...
set "CURL_OUTPUT_FILE=%GH_BACKUP_OUTPUT_TEMP_DIR%/%GH_RESTAPI_STARRED_REPOS_FILE%"

call set "CURL_OUTPUT_FILE=%%CURL_OUTPUT_FILE:{{PAGE}}=%PAGE%%%"

call "%%CONTOOLS_GITHUB_PROJECT_ROOT%%/tools/curl.bat" "%%GH_AUTH_USER%%" "%%GH_AUTH_PASS%%" "%%GH_RESTAPI_STARRED_REPOS_URL_PATH%%" || goto MAIN_EXIT
echo.
...
call set "GH_BACKUP_RESTAPI_STARRED_REPOS_FILE=%%GH_BACKUP_RESTAPI_STARRED_REPOS_FILE_NAME:{{OWNER}}=%OWNER%%%"
call set "GH_BACKUP_RESTAPI_STARRED_REPOS_FILE=%%GH_BACKUP_RESTAPI_STARRED_REPOS_FILE:{{DATE_TIME}}=%PROJECT_LOG_FILE_NAME_DATE_TIME%%%"

echo.Archiving backup directory...
call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/mkdir_if_notexist.bat" "%%GH_BACKUP_OUTPUT_DIR%%" && ^
call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/add_files_to_archive.bat" "%%GH_BACKUP_TEMP_DIR%%" "*" "%%GH_BACKUP_OUTPUT_DIR%%/%%GH_BACKUP_RESTAPI_STARRED_REPOS_FILE%%.7z" -sdel%%_7ZIP_BARE_FLAGS%% || exit /b 20
echo.
