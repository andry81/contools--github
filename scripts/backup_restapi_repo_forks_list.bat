...
set "CURL_OUTPUT_FILE=%GH_BACKUP_OUTPUT_TEMP_DIR%/%GH_RESTAPI_REPO_FORKS_FILE%"

call set "CURL_OUTPUT_FILE=%%CURL_OUTPUT_FILE:{{PAGE}}=%PAGE%%%"

call "%%CONTOOLS_GITHUB_PROJECT_ROOT%%/tools/curl.bat" "%%GH_AUTH_USER%%" "%%GH_AUTH_PASS%%" "%%GH_RESTAPI_REPO_FORKS_URL_PATH%%" || goto MAIN_EXIT
echo.
...
call set "GH_BACKUP_RESTAPI_REPO_FORKS_FILE=%%GH_BACKUP_RESTAPI_REPO_FORKS_FILE_NAME:{{OWNER}}=%OWNER%%%"
call set "GH_BACKUP_RESTAPI_REPO_FORKS_FILE=%%GH_BACKUP_RESTAPI_REPO_FORKS_FILE:{{REPO}}=%REPO%%%"
...
call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/add_files_to_archive.bat" "%%GH_BACKUP_TEMP_DIR%%" "*" "%%GH_BACKUP_OUTPUT_DIR%%/%%GH_BACKUP_RESTAPI_REPO_FORKS_FILE%%.7z" -sdel || exit /b 20
echo.
